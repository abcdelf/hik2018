...
void SquareGraph::setObstaclePos(vector<pair<int, int>> obstaclePos)
{
	for(auto p=obstaclePos.begin(); p != obstaclePos.end(); p++)
	{
		pair<int, int> coord = *p;
		setCellValue(coord,1);
		...
		for(auto j = 0;j<map.size();j++)
		{
			...
			}else
			{
				for(auto p=path.begin(); p != path.end(); p++)
				{
					Node node = *p;
					outStatus = 0;
					if(make_pair(i,j) == make_pair(node.x,node.y))
					{
